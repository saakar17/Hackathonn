...
APP_DIR = "/Users/saakar/AI-Assisted-Disease-Screening-for-Oral-Cancer/app"
...
def generate_pdf_single(patient_info: dict, report_df: pd.DataFrame, annotated_image: np.ndarray, file_path: str):
    ...
    # Pull info from first row of report_df
    row = report_df.iloc[0] if not report_df.empty else {}
    diagnosis = row.get("AI Diagnosis", "") if isinstance(row, dict) else row["AI Diagnosis"]
    confidence = row.get("Confidence Level", "") if isinstance(row, dict) else row["Confidence Level"]
    risk = row.get("Risk Assessment", "") if isinstance(row, dict) else row["Risk Assessment"]
    recommendation = row.get("Clinical Recommendation", "") if isinstance(row, dict) else row["Clinical Recommendation"]
    ...
    elements.append(Paragraph("_________________________________________<br/><br/>", block_label_style))
    # Build PDF
    doc.build(elements)
        ...
        if run_detection:
            ...
            try:
                with st.spinner("🔬 Analyzing image..."):
                    ...
                    with col2:
                        # Resize annotated image for webpage display (keep original image unchanged for PDF)
                        display_width = 400  # carefully chosen smaller width
                        try:
                            h, w = annotated_image.shape[:2]
                            if w > display_width:
                                scale = display_width / w
                                display_h = int(h * scale)
                                display_img = cv2.resize(annotated_image, (display_width, display_h), interpolation=cv2.INTER_AREA)
                            else:
                        ...
                        st.image(
                            display_img,
                            caption=f'AI Analysis - Patient: {patient_name}',
                            width=display_width,
